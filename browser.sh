...
case "$1" in
  chromedriver)
    ...
    # Use local chromedriver if available, otherwise system chromedriver
    CHROME_DRIVER="./chromedriver-local"
    if [ ! -f "$CHROME_DRIVER" ]; then
      CHROME_DRIVER="chromedriver"
    fi
    nohup "$CHROME_DRIVER" > "$LOG_FILE" 2>&1 &
    PID=$!
